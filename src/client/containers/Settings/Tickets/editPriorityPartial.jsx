...
class EditPriorityPartial extends React.Component {
  ...
  onSubmitEditPriority (e, priority) {
    ...
    const name = {
      'fr': e.target.namefr.value, 'en': e.target.nameen.value
    }
        ...
        <form onSubmit={e => this.onSubmitEditPriority(e, priority)}>
          <div className='uk-grid uk-grid-collapse uk-clearfix'>
            <div className='uk-width-1-5'>
              ...
              <input name={'namefr'} type='text' className={'md-input'} defaultValue={priority.get('name').get('fr')} />
            </div>
            <div className='uk-width-1-5'>
              <label>{t('Priority Name')} (en)</label>
              <input name={'nameen'} type='text' className={'md-input'} defaultValue={priority.get('name').get('en')} />
            </div>
